...
export default function Home() {
  ...
  const processRedemption = async (allCodes: string[]) => {
    ...
    const skippedStatuses: CouponStatus[] = skippedCodes.map((code) => {
      ...
      return {
        ...
        message: `Skipped: ${existing?.message || "Already processed"}`,
      };
    });
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-gradient-to-b from-slate-900 to-slate-800">
      <div className="flex flex-col items-center min-h-screen min-h-[100dvh] px-3 py-4 sm:px-4 sm:py-6 md:py-8">
        <div className="w-full max-w-md lg:max-w-lg">
          <div className="bg-slate-800/95 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-700/50 overflow-hidden">
            {/* Header */}
            <div className="text-center pt-4 sm:pt-6 md:pt-8 px-4 sm:px-6 md:px-8">
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1 sm:mb-2">
                Seven Knights Rebirth
              </h1>
              <p className="text-slate-400 text-sm sm:text-base">Coupon Auto Redeem</p>
            </div>

            {/* User ID Input - Outside Tabs */}
            <div className="px-4 sm:px-6 md:px-8 mt-4 sm:mt-6">
              <label htmlFor="uid" className="block text-xs sm:text-sm font-medium text-slate-300 mb-1.5 sm:mb-2">
                User ID (UID)
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-slate-700/80 border border-slate-600 rounded-lg sm:rounded-xl text-white text-sm sm:text-base placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
            ...
            <div className="flex mt-4 sm:mt-5 px-4 sm:px-6 md:px-8">
              <button
                ...
                className={`flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-medium rounded-t-lg border-b-2 transition-all ${
                  activeTab === "auto"
                    ? "bg-slate-700/50 text-white border-blue-500"
              ...
              {activeTab === "manual" && (
                <>
                  <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                    <div>
                      <label htmlFor="couponCode" className="block text-xs sm:text-sm font-medium text-slate-300 mb-1.5 sm:mb-2">
                        Coupon Codes
                      </label>
                      <textarea
                        id="couponCode"
                        ...
                        rows={3}
                        className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-slate-700/80 border border-slate-600 rounded-lg sm:rounded-xl text-white text-sm sm:text-base placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 uppercase resize-none"
                        required
                        disabled={isRedeeming}
                      />
                      <p className="mt-1 text-[10px] sm:text-xs text-slate-500">
                    ...
                    if (loadingStatus) {
                      return (
                        <div className="px-3 py-2 rounded-lg flex items-center gap-2 text-xs sm:text-sm bg-blue-500/10 border border-blue-500/30">
                          ...
                          <span className="font-mono text-white truncate">{loadingStatus.code}</span>
                          <span className="text-blue-400 text-[10px] sm:text-xs">Redeeming...</span>
                        </div>
                      );
                    }
                    ...
                    if (lastCompleted) {
                      return (
                        <div className={`px-3 py-2 rounded-lg flex items-center gap-2 text-xs sm:text-sm ${
                          ...
                          <span className="font-mono text-white truncate">{lastCompleted.code}</span>
                          <span className={`truncate text-[10px] sm:text-xs ${
                            lastCompleted.status === "success" ? "text-green-400" : "text-red-400"
                          }`}>
                ...
                <div>
                  {historyEntries.length > 0 ? (
                    <>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          ...
                          className="text-[10px] sm:text-xs text-slate-500 hover:text-red-400 active:text-red-500 transition-colors px-2 py-1"
                        >
                          Clear All
                        </button>
                      </div>
                      <div className="space-y-1.5 sm:space-y-2">
                        {historyEntries.map((entry, idx) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs sm:text-sm font-mono text-white truncate">{entry.code}</p>
                              <p className={`text-[10px] sm:text-xs truncate mt-0.5 ${
                            ...
                            <button
                              ...
                              className="flex-shrink-0 opacity-0 group-hover:opacity-100 sm:opacity-50 sm:hover:opacity-100 text-slate-500 hover:text-red-400 active:text-red-500 transition-all p-1"
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-slate-600 text-xs mt-1">Redeemed coupons will appear here</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {/* Footer */}
          <div className="text-center text-slate-500 text-[10px] sm:text-xs mt-3 sm:mt-4 px-4">
            <a
